...
case $choice in
    ...
    2)
        ...
        if [ -z "$DISPLAY" ] && [ -z "$WAYLAND_DISPLAY" ]; then
            echo "⚠️  警告: 未检测到图形显示环境"
            echo ""
            echo "这通常发生在以下情况："
            echo "• SSH连接到服务器但没有X11转发"
            echo "• 在Docker容器中运行"
            echo "• 在没有桌面环境的系统中运行"
            echo ""
            echo "解决方案："
            echo "1. 使用命令行模式: 选择选项3"
            echo "2. 启用X11转发: ssh -X username@hostname"
            echo "3. 在本地有图形界面的机器上运行"
            echo ""
            echo "是否尝试使用offscreen模式运行？(程序会运行但看不到界面) [y/N]"
            read -r response
            if [[ "$response" =~ ^[Yy]$ ]]; then
                echo "使用offscreen模式启动..."
                export QT_QPA_PLATFORM=offscreen
                timeout 10s ./robot_control_gui || echo "程序已超时退出"
            else
                echo "已取消启动。建议使用选项3启动命令行模式。"
            fi
        else
            ./robot_control_gui
        fi
